    ...
    export function swapCurrentPageWithNewHtml(html, andThen) {
      ...
      oldBodyScriptTagHashes = oldBodyScriptTagHashes.concat(
          Array.from(document.body.querySelectorAll('script'))
              .map(i => {return simpleHash(ignoreAttributes(
                       i.outerHTML, attributesExemptFromScriptTagHashing))}))

      let afterRemoteScriptsHaveLoaded =
          () => {}

                mergeNewHead(newHead)
                    .finally(() => {afterRemoteScriptsHaveLoaded()})
...
function mergeNewHead(newHead) {
  ...
  // @todo: Re-enable this code and find a better way to managed injected
  // stylesheets. See livewire/livewire#6824 for (let child of
  // Array.from(document.head.children)) {
  //     if (isAsset(child)) {
  //         if (! touchedHeadElements.some(i => i.outerHTML ===
  //         child.outerHTML)) {
  //             child.remove()
  //         }
  //     }
...
function isTracked(el) { return el.hasAttribute('data-navigate-track') }
...
function ifTheQueryStringChangedSinceLastRequest(el, currentHeadChildren) {
  ...
  return currentHeadChildren.some(child => {
    if (!isTracked(child))
      ...
      // Only consider a data-navigate-track element changed if the query string
      // has changed (not the URI)...
...
function isScript(el) { return el.tagName.toLowerCase() === 'script' }
